document.addEventListener('DOMContentLoaded', () => {
    fetch('../models/recommend_items.json')
        ...
        .then(data => {
            ...
            if (wrapper && data.recommend_items) {
                ...
                // Dynamically update grid columns based on actual item count
                const itemCount = data.recommend_items.length;
                const rows = 2;
                const totalCols = Math.ceil(itemCount / rows);
                const visibleMobile = 4.5;

                container.style.setProperty('--desktop-total-columns', totalCols);
                container.style.setProperty('--desktop-gap-count', totalCols - 1);
                container.style.setProperty('--tablet-total-columns', totalCols);
                container.style.setProperty('--tablet-gap-count', totalCols - 1);
                container.style.setProperty('--mobile-total-columns', totalCols);
                container.style.setProperty('--mobile-gap-count', Math.floor(visibleMobile));
